...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_FILES["csv_file_placed"])) {
    ...
    try {
        ...
        foreach ($dataRows as $data) {
            ...
            $offer_type    = isset($headerMap['offer_type']) ? trim($data[$headerMap['offer_type']] ?? 'Internship') : 'Internship';
            ...
            // If drive_no is empty, use a default value to avoid duplicate issues
            if (empty($drive_no)) {
                $drive_no = 'Import_' . date('Ymd_His') . '_' . $rowNumber;
            }
...
<div class="top-bar">
  ...
  <div class="left-controls">
    ...
    <a href="internship_placed_students" class="reset-button">
...
function applyFilters() {
  ...
  fetch("internship_placed_students", {
    method: "POST",
    body: formData
  })
  .then(res => res.text())
    ...
    .then(response => {
      ...
      if (response.success) {
        ...
        const promises = selectedIds.map(id => {
          return fetch(`internship_placed_students?ajax_row=1&place_id=${encodeURIComponent(id)}&t=${Date.now()}`)
  ...
  document.querySelectorAll(".row-save-btn").forEach(button => {
    button.addEventListener("click", () => {
      ...
      .then(response => {
        ...
        if (response.success) {
          fetch(`internship_placed_students?ajax_row=1&place_id=${encodeURIComponent(placeId)}&t=${Date.now()}`)
            .then(r => r.text())
            .then(html => {
              const temp = document.createElement("tbody");
              temp.innerHTML = html;
