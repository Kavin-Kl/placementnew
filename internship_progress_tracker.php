...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_single_row'])) {
    ...
    // Simplified internship tracker only writes the fields it owns. min_stipend lives on
    // Internship Company Data; the original follow_status / follow_up_person columns are
...
SELECT
  ...
  (
    ...
    WHERE
      ...
      AND ps.role = d.role
      AND ps.offer_type IN ('Internship','Apprenticeship (Part Time)','Internship + PPO (Pre-Final Year)')
...
<div class="heading-container7">
    <h3 class="headings">Internship Progress Tracker</h3>
    <p>Track pure Internship opportunities only. Manage SPOC info, hiring counts, status and follow-ups.</p>
