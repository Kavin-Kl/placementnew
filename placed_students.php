...
function render_student_row($row, $sl_no = '') {
    ...
    <tr data-student-id="<?= $row['student_id'] ?>" data-place-id="<?= $row['place_id'] ?>">
        ...
        <td><?= htmlspecialchars($row['offer_type'] ?? 'Not Set') ?></td>
        ...
        <td><input type="text" class="edit_ctc" value="<?= htmlspecialchars($row['edit_ctc'] ?? $row['ctc'] ?? '') ?>"></td>
        <td><input type="text" class="edit_stipend" value="<?= htmlspecialchars($row['edit_stipend'] ?? $row['stipend'] ?? '') ?>"></td>
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_FILES["csv_file_placed"])) {
    ...
    if (!in_array($fileExt, $allowedTypes)) {
        ...
        header("Location: placed_students.php");
        exit;
    }
    ...
    try {
        ...
        $skippedDetails = [];
        $rowNumber = 2; // row 1 is the header in Excel
        ...
        foreach ($dataRows as $data) {
            ...
            $offer_type    = trim($data[$headerMap['offer_type']] ?? 'FTE'); // Default to FTE for final year
            ...
            // For final year placements, default to FTE if not specified
            if (empty($offer_type) || $offer_type === 'Internship') {
                $offer_type = 'FTE';
            }
            ...
            // If student doesn't exist, auto-create them in `students` so we don't force the user
            // to do a separate registered-students import for placement-only data.
            if (!$student_id) {
                ...
                if ($createStu) {
                    ...
                    if ($createStu->execute()) {
                        ...
                        error_log("Auto-create student failed for UPID $upid: " . $createStu->error);
                    }
                    $createStu->close();
                }
                ...
                if (!$student_id) {
                    ...
                    error_log("Skipping placement record - could not auto-create student with UPID '$upid'");
                    $rowNumber++;
                    continue;
                }
            }
            ...
            if ($checkPlaced) {
                ...
                if ($checkPlaced->num_rows > 0) {
                    ...
                    $skippedDetails[] = "Row $rowNumber: Duplicate placement for UPID '$upid' with drive_no '$drive_no'";
                    $checkPlaced->close();
                    $rowNumber++;
                    continue;
                }
            ...
            if ($stmt) {
                $stmt->bind_param("iiissssssssdsssssss",
                ...
                if ($stmt->execute()) {
                    ...
                    $skipped++;
                    $skipReasons['errors']++;
                    $skippedDetails[] = "Row $rowNumber: Database error for UPID '$upid' — " . $stmt->error;
                }
        ...
        if ($drives_created > 0 || $roles_created > 0) {
            $message .= ". Auto-created $drives_created drive(s) and $roles_created role(s) — they now appear in the Progress Tracker and Company Data tabs.";
        }

        $_SESSION['import_message'] = $message;
        if (!empty($skippedDetails)) {
            $_SESSION['import_details'] = count($skippedDetails) <= 10
                ? implode('; ', $skippedDetails)
                : implode('; ', array_slice($skippedDetails, 0, 10)) . "; ... and " . (count($skippedDetails) - 10) . " more issues";
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_filter']) && $_POST['ajax_filter'] === "1") {
    ...
    // Resolve final-year integer from selected academic year (e.g. "2026-2027" -> 2027)
    $final_year_int = null;
    ...
    // Add academic year filter on placement row (s.academic_year)
    if (!empty($_SESSION['selected_academic_year'])) {
    ...
    foreach ($allFields as $field) {
        if ($field === 'course') {
            ...
            if (!empty($_POST['course']) && is_array($_POST['course'])) {
                ...
                $where[] = "ps.course IN (" . implode(',', $placeholders) . ")";
            }
    ...
    // Tab routing for full-time placed:
    // Default: anything NOT in the internship bucket lands here. The internship bucket is:
    //   - offer_type = 'Internship'
    //   - offer_type = 'Apprenticeship (Part Time)'
    //   - 'Internship + PPO' for non-final-year students
    // Tolerant of legacy/blank/NULL offer_types so old imports stay visible.
    $route_clause = $final_year_int
        ? "NOT (
                ...
                OR (
                  ...
                  AND s.year_of_passing <> $final_year_int
                )
            )"
...
if (!empty($_SESSION['import_message'])) {
    ...
    if (!empty($_SESSION['import_details'])) {
        $messageHtml .= "<details class='$class' style='margin-top:8px;'><summary style='cursor:pointer;'>Show details</summary><div style='margin-top:6px; font-size:13px; white-space:pre-wrap;'>"
            . htmlspecialchars($_SESSION['import_details'])
            . "</div></details>";
...
<div class="table-responsive" style="max-height: calc(100vh - 220px); min-height: 60vh; overflow-y: auto;">
  <table class="table table-bordered table-striped custom-table sticky-table" id="placed_studentsTable">
    ...
    <tbody id="tableBody">
      ...
      // Resolve final-year integer + AY string for the default listing query
      $ps_ay = $_SESSION['selected_academic_year'] ?? null;
      $ps_ay_parts = $ps_ay ? explode('-', $ps_ay) : [];
      $ps_final_year_int = isset($ps_ay_parts[1]) ? intval($ps_ay_parts[1]) : null;
      $ps_ay_sql_join = " LEFT JOIN students s ON ps.student_id = s.student_id ";
      $ps_ay_where = $ps_ay
          ? " AND s.academic_year = '" . $conn->real_escape_string($ps_ay) . "' "
          : "";
      $ps_route_clause = $ps_final_year_int
        ? "NOT (
              ps.offer_type IN ('Internship','Apprenticeship (Part Time)','Internship + PPO (Pre-Final Year)')
              OR (
                ps.offer_type IN ('Internship + PPO','Internship+PPO')
                AND s.year_of_passing IS NOT NULL
                AND s.year_of_passing <> $ps_final_year_int
              )
          )"
        : "(ps.offer_type IS NULL OR ps.offer_type NOT IN ('Internship','Apprenticeship (Part Time)','Internship + PPO (Pre-Final Year)'))";

      $ps_count_q = $conn->query("
        SELECT COUNT(*) AS c FROM (
          SELECT DISTINCT ps.student_id, ps.drive_no
          FROM placed_students ps
          $ps_ay_sql_join
          WHERE $ps_route_clause
            AND ps.program IS NOT NULL
            AND ps.program != ''
            $ps_ay_where
        ) t
      ");
      $ps_total = (int)($ps_count_q ? $ps_count_q->fetch_assoc()['c'] : 0);
      $ps_pg = paginate_setup($ps_total, 10, 'page');
      $defaultQuery = $conn->query("
        ...
        LIMIT {$ps_pg['per_page']} OFFSET {$ps_pg['offset']}
      ");
      if (!$defaultQuery) {
          echo "<tr><td colspan='20'>Error: " . $conn->error . "</td></tr>";
      } else {
          $count = $defaultQuery->num_rows;
          $sl_no = $ps_pg['offset'] + 1;
          while ($row = $defaultQuery->fetch_assoc()) {
              echo render_student_row($row, $sl_no++);
          }
          if ($count == 0) {
              echo "<tr><td colspan='20' style='text-align:center; padding:20px;'>No placed students found.</td></tr>";
          }
      }
      ?>
</tbody>
...
function attachRowSaveEvent() {
  document.querySelectorAll(".row-save-btn").forEach(button => {
    button.addEventListener("click", () => {
      ...
      const data = {
        place_ids: [placeId],
        offer_type: row.querySelector(".offer_type")?.value || "",
